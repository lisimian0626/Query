...
import com.beidousat.querydata.widget.WidgetCashPager;
import com.beidousat.querydata.widget.WidgetStorePager;
...
public class StoreActivity extends BaseActivity implements NavigationView.OnNavigationItemSelectedListener, View.OnClickListener, StationConstract.View, StoreConstract.View,OnPageScrollListener, WidgetPage.OnPageChangedListener  {
    DrawerLayout mDrawerLayout;
    Toolbar mToolbar;
    NavigationView mNavigationView;
    private ActionBarDrawerToggle mDrawerToggle;
    private WidgetPage mWidgetPage;
    private WidgetStorePager widgetStorePager;
    ...
    public void initViews() {
        ...
        widgetStorePager = findViewById(R.id.storePaper);
...
//        layout_head=findViewById(R.id.recharge_head);
        ...
        widgetStorePager.setOnPagerScrollListener(this);
        //设置toolbar
...
//        mDrawerLayout.setDrawerListener(mDrawerToggle);
    ...
    private void requestStore() {
        ...
        storePresenter.getStoreList(requestParams, 1, Constant.per_pager);
    }
...
//            mToolbar.setTitle("首页");
    ...
    public void initStorePager(int totalPage, List<Store.RootBean.DataBean> dataBeanList,Map<String, String> params) {
        L.i(getClass().getSimpleName(), "Current total page:" + totalPage);
        mWidgetPage.setPageCurrent(0);
        mWidgetPage.setPageTotal(totalPage);
        widgetStorePager.initPager(totalPage, dataBeanList, params);
    }

    @Override
    public void onPrePageClick(int before, int current) {
        widgetStorePager.setCurrentItem(current);
    }
    ...
    public void onPagerSelected(int i, boolean b) {
        widgetStorePager.runLayoutAnimation();
    ...
    public void OnRequestData(Store store) {
        if (store != null && store.getRoot() != null && store.getRoot().getData() != null && store.getRoot().getData().size() > 0) {
            initStorePager(Integer.valueOf(store.getRoot().getTotal()),store.getRoot().getData(), requestParams);
        }else{
            initStorePager(0,null,requestParams);
        }
    }
}
